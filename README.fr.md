...
- Le regroupement ACP applique maintenant cette corrélation sur la fermeture 24h IP/cookie autour des sessions affichées, puis trie les groupes par **dernière activité observée** du groupe fusionné.
...
- Timeout session.
- Configurer le chemin du log sécurité (défaut : `/var/log/security_audit.log`). Mettre à jour les jails fail2ban si le chemin est modifié.
- Seuils de détection navigateurs / absence JS.

Checklist production:

- Vérifier que le process PHP (`www-data`) peut écrire dans le chemin configuré.
- Vérifier que le `logpath` de chaque jail fail2ban correspond au chemin configuré.
...
- En cas de login raté, seul le **login soumis** et le **code d'erreur auth** sont stockés; le mot de passe n'est jamais collecté.
